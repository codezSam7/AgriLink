<?php 
  ...
  //check if recipe_id is not inside query string;
  ...
  if($recipe_dey_before == false){
    //recipe never dey inside table: call a method that will add it
    $added = $carts->add_to_cart($product_id, $buyer_id, 1);
  }else{
    //receipe dey inside table: call a method that will update it
